...
const CustomSelectFilter = ({ name, dataList, setPage, activeClick, changeActiveFilter, type = 'checkbox' }) => {
    ...
    return (
        <div
            id={`order_filter_${name}`}
            onClick={() => changeActiveFilter(activeClick !== name ? name : null)}
            className={
                `relative p-3 gap-x-5 border-[1px] border-main flex justify-between items-center cursor-pointer select-none rounded-md transition-all ` +
                (activeClick === name ? 'bg-main text-white' : 'text-gray-500')
            }
        >
            <span id={`order_label_filter_${name}`} className='capitalize'>
                {name}
            </span>
            <i id={`order_icon_filter_${name}`} className='fa-solid fa-angle-down'></i>
            {activeClick === name && (
                <div className='absolute border border-main z-20 top-[calc(100%+2px)] left-0 bg-white shadow-lg rounded-md animate-fade-in-fwd'>
                    {type === 'checkbox' && (
                        <>
                            <div className='p-4 text-gray-500 flex items-center justify-between font-semibold gap-x-8'>
                                <span className='whitespace-nowrap'>{selected.length} selected</span>
                                <span onClick={() => setSelected([])} className=' hover:text-red-500 cursor-pointer'>
                                    Reset
                                </span>
                            </div>
                            <ul className='pb-4 px-2 max-h-[400px] overflow-auto'>
                                {dataList?.map(item => (
                                    <li
                                        ...
                                        key={item.key}
                                        className='py-2 px-4 hover:bg-main hover:text-white flex items-center rounded-md capitalize'
                                    >
                                        <input
                                            ...
                                            type='checkbox'
                                            value=''
                                            className='w-4 h-4 text-main bg-gray-100 border-gray-300 rounded focus:ring-sky-500 cursor-pointer'
                                        />
                                        <label
                        ...
                        <>
                            ...
                            <div className='flex gap-x-5 px-5 py-3 min-w-[500px]' onClick={e => e.stopPropagation()}>
                                ...
                                <InputFields
                                    ...
                                    invalid={invalid}
                                    setInvalid={setInvalid}
                                    autoFocus={price?.maxPrice.length === 0 ? false : true}
                            ...
                            <ul className='p-2 text-gray-500 text-nowrap shadow-lg'>
                                {dataList?.map(item => (
                                    <li
                                        ...
                                        key={item.key}
                                        className='py-2 px-3 rounded hover:bg-main hover:text-white group'
                                    >
                                        <span className='min-w-[150px] inline-block'>{item.value}</span>
                                        {selected === item.key && (
                                            <i className='fa-solid fa-check mr-2 font-extrabold text-main group-hover:text-white'></i>
                                        )}
                                    </li>
                                ))}
                            </ul>

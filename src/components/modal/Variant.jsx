...
import { addVariant, deleteVariant, getProductDetail, updateProduct } from '../../apis'
            ...
            <div className="relative w-auto h-fit mx-auto my-10 max-w-screen-lg">
                <div className="border-[2px] border-gray-400 rounded-xl shadow-xl min-w-[600px] relative flex flex-col bg-white outline-none focus:outline-none animate-slide-in-blurred-top">
                    <div className="flex items-center justify-between px-4 py-3 border-b border-solid border-gray-300 rounded-t ">
                        ...
                        <div className=''>
                            <button disabled={isLoading} onClick={() => setDefaultValue()} className={`px-5 py-2 mr-5 bg-main rounded-md text-white ${isLoading && 'opacity-80'}`}>
                    ...
                    <div className='overflow-auto max-h-[calc(100vh-80px-72px)]'>
                        {variant.length > 0 && variant.map((item, index, arr) => (
                            <div className={`relative px-6 mt-5 flex-auto text-black`}>
                                <div className='grid grid-cols-5 items-center gap-x-5'>
                                    <div>
                                        ...
                                        <input disabled className={classInput} value={item?.variantType} placeholder="Color" />        
                                    </div>
                                    <div>
                                        <label htmlFor="">Name:</label>
                                        <input disabled type='text' placeholder='Black' className={classInput} value={item?.name}/> 
                                    </div>
                                    <div>
                                    ...
                                    <div className='flex gap-x-5 justify-center'>
                                        ...
                                        <button onClick={() => handleDeletevariant(item)} className={'px-5 py-2 rounded-lg border-2 border-red-500 text-red-500 hover:bg-red-500 hover:text-white'}><i className="fa-regular fa-trash-can"></i></button>
                                    </div>
                                </div>
                    ...
                    <div className="my-5 px-6">
                        <form onSubmit={handleSubmit(onSubmit)}>
                            <div className='flex items-center gap-x-5 mb-3'>
                                ...
                                <button
                                    disabled={isLoading}
                                    className={`border-2 min-w-[120px] border-gray-500 block text-gray-500 font-bold uppercase px-6 py-3 rounded text-sm outline-none focus:outline-none mr-1 ${isLoading ? 'opacity-90' : 'hover:text-white hover:bg-gray-500'}`}
                                    type="button"
                                ...
                                <div className='w-[80%] flex flex-col'>
                                    <div className='grid grid-cols-2 gap-x-5'>
                                        <div>
                                            ...
                                            {errors.variantType && <span className={classError}>{errors.variantType.message}</span>}
                                        </div>
                                        <div>
                                            <label htmlFor="">Name:</label>

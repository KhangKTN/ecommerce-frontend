                    ...
                    <div className='relative px-3 flex-auto text-black'>
                        <div className='my-3'>
                            {current?.address.map((item) => (
                                <div
                                    key={item._id}
                                    className='hover:bg-gray-100 rounded-md p-3 flex flex-col border-b-[1px]'
                                >
                                    <div className='flex gap-x-3 items-center mb-2'>
                                        ...
                                        <span
                                            className='text-main font-medium ml-auto cursor-pointer hover:underline'
                                            onClick={() => handleUpdate(item)}
                                        >
                                            Update
                                        </span>
                                    </div>
                                    <span className='text-gray-500'>{`${localAddressToString(item.localAddress)}, ${
                                        item.detailAddress
                                    }`}</span>
                                    {item.isDefault && (
                                        <span className='border-[1px] border-main w-fit px-2 py-1 rounded'>
                                            Default
                                        </span>
                                    )}
                                </div>
                        ...
                        <button
                            className='border-2 text-main font-medium border-main px-4 py-2 rounded'
                            onClick={() => {
                                setModalAddress(false)
                                setModalAddAddress(true)
                            }}
                        >
                            <i className='fa-solid fa-plus mr-3'></i>Add new address
                        </button>
                    </div>

...
const Product = ({product, showInfo, logo}) => {
    ...
    return (
        <Link to={`/${product?.category?.toLowerCase()}/${product?._id}/${product?.slug}`} onMouseEnter={() => setIsHover(true)} onMouseLeave={() => setIsHover(false)} className='px-[10px] block mt-8'>
            <div className='border-[1px] border-gray-300 h-[385px] rounded-lg relative transition-all duration-[500]'>
                {logo &&
                    <img
                        className='absolute -top-8 right-0 z-10 rotate-12 w-20 h-20' 
                        src={logo}
                    />
                }
                        ...
                        <div className={'absolute z-10 top-0 right-0 left-0 flex flex-col gap-x-3 ' + (isHover ? 'animate-slide-in-bottom' : 'animate-slide-to-bottom') + (showInfo ? ' bg-[#ffffff8c] opacity-50 backdrop-blur-md flex-col bottom-0 justify-between' : ' items-end -bottom-[10px] justify-end')}>
                            ...
                            <>
                                ...
                                <div className='mt-3 ml-3'>
                                    {product?.description.map((item, index) => (
                                        index < 10 && <h1 key={index} className='text-sm'>{item}</h1>
                                    ))}
                                </div>
                            </>
                            }
                            <div className='flex gap-x-3 mx-auto mt-5'>
                                {/* <Tippy className='text-main' content={<span></span>}> */}
                                    <Link onClick={async(e) => {await addWishList(product._id); e.stopPropagation(); dispatcher(getCurrentUser())}}>
                                        <ActionButton color={isFavourite() ? 'bg-pink-500 text-white' : null} icon={<i className="fa-regular fa-heart"></i>}/>
                                    </Link>
                                {/* </div></Tippy> */}
                ...
                <div className='flex flex-col px-4 mt-4 gap-y-2 relative'>
                    <div className={showInfo && isHover ? 'absolute top-0 right-0 bottom-0 left-0 bg-[#ffffffd4] opacity-90 backdrop-blur-md' : ''}></div>

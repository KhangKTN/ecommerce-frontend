...
const Sidebar = () => {
    ...
    return(
        <div className="border-[1px] h-fit border-gray-300 rounded-md overflow-hidden">
            <div className="uppercase py-[15px] px-5 bg-main text-white font-semibold"><i className="fa-solid fa-list mr-3"></i>All Collection</div>
            {categories?.map((item, index) => (
                <NavLink key={item._id} className={({isActive}) => isActive ? itemActive : itemNotActive} to={item.name.toLowerCase()}>
                    <div>{item?.name}</div>
                </NavLink>
            ))}
        </div>

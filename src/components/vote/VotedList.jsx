        ...
        <>
            <div className='flex gap-x-5'>
                <img
                    ...
                    src='https://lh5.googleusercontent.com/proxy/b7aE9Mn5FCMO2HDvuyuUFFRONGzX3Cerpp-BiPfPzcoGhJDzpHc7DNARfVNp-oSZ14Yww9XR_zihJ9WEHrlKIqjqXM8XL9J8RZ8vGhCTYttooJO73nudWbDNqIMuqGFO'
                    alt=''
                />
                <div key={voteData?.voteBy?._id} className=''>
                    ...
                    <div className='flex gap-x-5'>
                        ...
                        {!replyData && current?.role === '1963' && (
                            <span
                                onClick={() => setShowFormReply(true)}
                                className='text-main italic font-bold cursor-pointer hover:underline'
                            >
                                Reply
                            </span>
                        )}
                    ...
                    <div className='flex gap-x-3 mt-3'>
                        {voteData?.images?.map((img, index) => (
                            <div
                                key={img}
                                className={`size-[64px] ${currentImg === index && 'p-[1px] border-2 border-main'}`}
                            >
                                <img
                                    key={img}
                    ...
                    {showFormReply && (
                        <div className='bg-gray-200 rounded mt-3 p-5 animate-fade-in-fwd'>
                            ...
                            {replyContent.trim().length < 10 && (
                                <small className='text-red-500'>Please input least 2 words and 10 characters!</small>
                            )}
                            <div className='flex justify-end gap-x-3 mt-5 text-main font-semibold'>
                                <button
                                    onClick={() => setShowFormReply(false)}
                                    className='pl-5 pr-4 py-2 border-2 border-main rounded-md'
                                >
                                    Cancel
                                </button>
                                <ButtonLoading
                                    ...
                                    disabled={
                                        !(
                                            replyContent.trim().length >= 10 &&
                                            replyContent.trim().split(' ').length >= 2
                                        )
                                    }
                                />
                            </div>

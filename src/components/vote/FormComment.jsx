            ...
            <div className='flex flex-col gap-y-5'>
                ...
                <div className='basis-3/5'>
                    <div className='text-right'>
                        <span onClick={() => {setImages([]); setPreviewImg([])}} className='cursor-pointer hover:text-red-500 hover:underline font-medium'>Reset photos</span>
                    </div>
                    {images?.length > 0 ?
                        <div className='grid grid-cols-4 gap-3'>
                            {previewImg?.map((e, index) => 
                                <div key={e} onMouseEnter={() => setImgHover(index)} onMouseLeave={() => setImgHover(null)} className='relative'>
                                    ...
                                    {imgHover === index && <button onClick={() => handleDeleteImage(index)} className='absolute flex items-center justify-center top-1 right-1 bg-[#ffffff45] backdrop-blur-md size-8 rounded-full hover:text-white hover:bg-red-500 transition-all duration-300'><i className="fa-solid fa-xmark"></i></button>}
                                </div>
                            )}
                            {images.length < 4 && 
                    ...
                    : 
                        <div className='bg-gray-100 border-[1px] border-gray-300 w-full h-[120px] rounded-md'>
                            <label className='font-semibold text-lg cursor-pointer flex flex-col justify-center' htmlFor="uploadFile">
                                ...
                                <span className='text-sm text-center text-gray-500'>Add descriptive images to your products <br/>(Upload maximum 4 images)</span>
                            </label>
                        </div> 
                    }

                    ...
                    <div className='flex p-4 md:p-5 space-y-4'>
                        <div className='relative w-[6/10] h-full flex-1'>
                            <span
                                ...
                                className='absolute top-1/2 -translate-y-1/2 right-0 text-2xl rounded-full flex items-center justify-center cursor-pointer text-main size-10 hover:bg-white'
                            >
                                <i className='fa-solid fa-chevron-right'></i>
                            </span>
                            <span
                                onClick={() => handleSlide('')}
                                className='absolute top-1/2 -translate-y-1/2 left-0 text-2xl rounded-full flex items-center justify-center cursor-pointer text-main size-10 hover:bg-white'
                            >
                                <i className='fa-solid fa-chevron-left'></i>
                            </span>
                            <img
                                className='w-full h-[450px] object-contain'
                                src={listImage[indexCurrentImg]}
                                alt='currentImg'
                            />
                        </div>
                        ...
                        <div className='w-[4/10] grid grid-cols-3 gap-3 h-fit'>
                            {listImage?.map((item, index) => (
                                <div className='h-[80px]'>
                                    <img
                                        onClick={() => setIndexCurrentImg(index)}
                                        key={index}
                                        className={
                                            `h-[80px] w-[80px] object-contain cursor-pointer ` +
                                            (index === indexCurrentImg ? 'border-2 border-sky-500' : '')
                                        }
                                        src={item}

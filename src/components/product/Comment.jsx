...
import { Rating } from '.'
import { getInfoVoteSummary, getVoting } from '../../apis/product'
            ...
            <div className='bg-white px-3 py-4 rounded-md'>
                <div className='rounded-md' ref={ref} onClick={() => (ref.current.className = 'rounded-md')}>
                    {Array.from({ length: 5 }, (x, i) => i).map((e, index) => (
                        <div
                            key={index}
                            onClick={() => setVoteSelected(e + 1)}
                            className={'rounded-md overflow-hidden'}
                        >
                            <Votebar
                                value={index}
                                voted={infoSummary?.countStar[`star_${index + 1}`]}
                                totalVote={infoSummary?.totalVote}
                            />
                        </div>
                    ))}
                </div>
                ...
                <div className='bg-[#d4e7f6] p-5 rounded-md mt-3 flex gap-x-5'>
                    <div className='min-w-fit'>
                        <h1 className='text-main font-semibold text-3xl'>
                            <span>{infoSummary?.averageVote} / </span>5{' '}
                            <span className='text-base text-gray-800'>({infoSummary?.totalVote} ratings)</span>
                        </h1>
                    ...
                    <div className='flex gap-4 flex-wrap basis-full'>
                        {filterReview?.map((item) => (
                            <div
                                ...
                                key={item.id}
                                className={
                                    'h-fit py-2 px-4 border-[1px] cursor-pointer bg-white text-gray-600 rounded text-nowrap ' +
                                    (selected === item.id ? 'border-[#5badec] text-main' : 'border-gray-400')
                                }
                            >
                                <h1>
                                    {item.text} {item.value && `(${infoSummary?.countStar[item.id]})`}
                                </h1>
                            </div>
                        ))}
                    </div>

...
const Blog = () => {
    ...
    useEffect(() => {
        ...
        if (!isLoadFirst) {
            ...
            if (search) queries.s = search
            else delete queries.s
            navigate({
                ...
                <div className='flex gap-x-5 justify-center'>
                    {linkBlog.map(item => (
                        <NavLink
                            key={item.id}
                            to={
                                search.trim().length === 0 ? `/blogs/${item.value}` : `/blogs/${item.value}?s=${search}`
                            }
                            className={({ isActive }) => (isActive ? actClass : noActClass)}
                        >
                            {item.text}
                        </NavLink>
                    ))}

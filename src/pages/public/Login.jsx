                ...
                <div className="w-full max-w-md bg-white rounded-xl shadow-md p-6">
                    <div className="flex flex-wrap -mx-3 mb-6">
                        <div className="w-full md:w-full px-3 mb-6">
                            <InputFields name='email' type='email' value={loginInfo} setValue={setLoginInfo} invalid={invalid} setInvalid={setInvalid} autoFocus={true} placeHolder='Email'/>
                        </div>
                        <div className="w-full md:w-full px-3 mb-6">
                            <div className="w-full relative">
                                <InputFields name='password' value={loginInfo} setValue={setLoginInfo} invalid={invalid} setInvalid={setInvalid} placeHolder='Password' type='password' isPassword={true}/>
                            </div>
                        </div>
                        <div className="w-full flex items-center justify-between px-3 mb-3 ">
                            ...
                            <div className="w-1/2 text-right">
                                <Link to={`/${path.FORGOT_PASSWORD}?step=1`} className="text-blue-500 text-sm tracking-tight">Forget your password?</Link>
                            </div>
                        </div>
                        <div className="w-full md:w-full px-3 mb-6">
                            <button onClick={() => handleClick()} className="w-full bg-blue-600 border border-gray-200 rounded-lg py-3 px-3 leading-tight bg-gradient-to-r hover:from-teal-400 hover:to-blue-500  text-white font-semibold !hover:transition-all !hover:duration-500">Sign in</button>
                            <span className="text-center block mx-auto mt-3">
                                If you don't have an account?
                                <Link to={`/${path.REGISTER}`} className="text-blue-500"> Register now</Link>
                            </span>
                            <div>
                                <span className="text-center block mx-auto mt-3">
                                    <Link to={`/${path.HOME}`} className="text-blue-500">Back to Homepage</Link>
                                </span>
                            </div>
                        </div>

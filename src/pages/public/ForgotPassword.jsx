...
const ForgotPassword = () => {
    ...
    const [invalid, setInvalid] = useState(defaultObj)
    ...
    const handleClick = useCallback(async() => {
        if(step == 1){
            ...
            if(data.password !== data.repassword){
                ...
                // setData(prev =>  ({...prev, token: query.get('token')}))
                setIsLoading(true)
                const dataSend = {...data, token}
                const res = await resetPassword(dataSend)
                console.log(res);
                setIsLoading(false)
                if(res.success){
                    navigate(`/${path.LOGIN}`)
    ...
    useEffect(() => {
        if((step != 1 && step != 2) || (step == 2 && !token)) navigate(`/${path.HOME}`)
    }, [])
                    ...
                    {query.get('step') == 1 ?
                        <div className="w-full md:w-full mb-3">
                            ...
                            <div className="w-full">
                                <InputFields name='email' value={data} setValue={setData} invalid={invalid} setInvalid={setInvalid} autoFocus={true} placeHolder='your_email@domain.com' type='text'/>
                            </div>
                        </div>
                    :
                        <>
                            <div className="w-full md:w-full mb-3">
                                <div className="w-full relative">
                                    <InputFields name='password' value={data} setValue={setData} invalid={invalid} setInvalid={setInvalid} type='password' placeHolder='Password' isPassword={true}/>
                                </div>
                            </div>
                            <div className="w-full md:w-full mb-3">
                                <div className="w-full relative">
                                    <InputFields name='repassword' value={data} setValue={setData} invalid={invalid} setInvalid={setInvalid} type='password' placeHolder='Comfirm password' isPassword={true}/>
                                </div>
                            </div>
                        </>
                    }

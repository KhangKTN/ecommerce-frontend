...
const CreateBlog = ({ isEdit, blog, handleReload }) => {
    ...
    const handleSave = async () => {
        setIsLoading(true)
        let res = null
        if (isEdit) res = await updateBlog({ _id: blog._id, title, category, content })
    ...
    return (
        <div>
                ...
                <div>
                    ...
                    <select onChange={(e) => setCategory(e.target.value)} className={inputClass} name='' id=''>
                        {linkBlog.map(
                            (item, index) =>
                                index !== 0 && (
                                    <option key={item.id} selected={item.value === category} value={item.value}>
                                        {item.text}
                                    </option>
                                )
                        )}
            ...
            <div className='mt-5 w-fit mx-auto'>
                <ButtonLoading handleClick={handleSave} text='Save' isLoading={isLoading} />
            </div>
        </div>
    )
}

...
const ManageProduct = () => {
    ...
    return (
        <div>
            ...
            <div ref={refTable} className="flex justify-between items-baseline sm:rounded-lg mt-10 mb-5">
                ...
                <div className="order-2 mb-0 flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 py-4 bg-white dark:bg-gray-900">
                    ...
                    <div className="relative">
                        ...
                        <input type="search" value={search} onChange={(e) => {setSearch(e.target.value); isLoadFirst && setIsLoadFirst(false)}} className="block py-2 ps-10 pe-2 text-sm outline-none text-gray-900 border border-gray-300 rounded-lg w-80 bg-gray-50 focus:border-main dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search by name..."/>
                    </div>
                </div>
                {/* Filter product */}
                ...
                <table className="w-full text-sm text-left table-auto shadow-lg rtl:text-right text-gray-500 dark:text-gray-400">
                        ...
                        <tr>
                            <th scope="col" className="px-3 py-4">
                                <div className="flex items-center">
                                    <input checked={selected.length === productList?.length && productList?.length !== 0} onClick={() => setSelected(selected.length === productList.length ? [] : productList?.map(item => item._id))} id="checkbox-all-search" type="checkbox" className="w-4 h-4 bg-gray-100 border-gray-300 rounded"/>
                    ...
                    <tbody>
                        {productList?.map(item => (
                            <tr key={item._id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 bg-white border-b text-base dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600">
                                <td className="w-4 pl-3">
                                    <div className="flex items-center">
                                        <input onClick={() => handleClickCheckbox(item._id)} checked={selected?.includes(item._id)} id="checkbox-table-search-1" type="checkbox" className="w-4 h-4 border-gray-300 rounded focus:ring-0 dark:focus:ring-blue-600 dark:ring-offset-gray-800 dark:focus:ring-offset-gray-800 dark:bg-gray-700 dark:border-gray-600"/>
                                        <label for="checkbox-table-search-1" className="sr-only">checkbox</label>
                                    </div>
                                </td>
                                <th scope="row" className="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                    <img className="w-20 h-16 object-contain rounded-md" src={item.thumbnail} alt="Jese image"/>
                                    <div className="ps-3">
                                        <div className="text-base font-semibold"></div>
                                    </div>
                                </th>
                                <td className="px-6 py-4 truncate font-semibold">
                                    {item.name}
                                </td>
                                ...
                                <td>
                                    <div className='flex gap-x-3 items-center '>
                                        <Tippy className='text-main bg-gray-700' content={<span>Edit product</span>}>
                                            <button onClick={() => {setCurrentProduct(item); setShowModalUpdate(true)}} className={'px-5 py-2 rounded-lg border-2 border-green-500 text-green-500 hover:bg-green-500 hover:text-white'}><i className="fa-solid fa-file-pen"></i></button>
                                        </Tippy>
                                        <Tippy className='text-main bg-gray-700' content={<span>Variant product</span>}>
                                            <button onClick={() => {setCurrentProduct(item); setShowModalVarient(true)}} className={'px-5 py-2 rounded-lg border-2 border-purple-400 text-purple-400 hover:bg-purple-400 hover:text-white'}><i className="fa-solid fa-diagram-project"></i></button>
                                        </Tippy>
                                        <Tippy className='text-main bg-gray-700' content={<span>Delete product</span>}>
                                            <button onClick={() => {setCurrentProduct(item); setModalShow(true)}} className={'px-5 py-2 rounded-lg border-2 border-red-500 text-red-500 hover:bg-red-500 hover:text-white'}><i className="fa-regular fa-trash-can"></i></button>
                                        </Tippy>
                                    </div>
                                </td>
                            </tr>
            ...
            {showModal && <ConfirmModal setShowModal={setModalShow} setCurrent={setCurrentProduct} title={'Confirm delete product'} content={`Do you want delete product ${currentProduct?.name}?`} handleClick={handleDeleteProduct}/>}
            {showModalUpdate && <UpdateProduct title='Update Product' setShowModal={setShowModalUpdate} product={currentProduct} handleRefresh={fetchProductList}/>}
            {showModalVarient && <Varient product={currentProduct} setShowModal={setShowModalVarient} setCurrent={setCurrentProduct} handleRefresh={fetchProductList} title={`List variant ${currentProduct?.name}`} /> }
        </div>
    )
}

...
import { getDistrictByCode, getProvinceByCode, getWardByCode } from 'vn-local-plus'
...
const Checkout = () => {
    ...
    const localAddressToString = (address) => {
        ...
        return `${getProvinceByCode(localAddress[0]).name}, ${getDistrictByCode(localAddress[1]).name}, ${
            getWardByCode(localAddress[2]).name
        }`
    }
                ...
                <>
                    <div className='mt-5 rounded-lg pt-[4px] bg-gradient-to-r from-[#00c6fb] to-[#005bea]'>
                        <div className='py-5 px-8 rounded-b-lg bg-gray-100'>
                            <h1 className='text-xl font-semibold text-main mb-3'>
                                <i className='fa-solid fa-location-dot mr-3'></i>Delivery Address
                            </h1>
                            <div className='flex gap-x-5 items-center'>
                                <h1 className={'text-lg font-semibold ' + (!address && 'text-red-400')}>
                                    {address
                                        ? `${address?.name} | ${address?.phone}`
                                        : 'Please add an address for delivery!'}
                                </h1>
                                <span className=''>
                                    {localAddressToString(address?.localAddress)}, {address?.detailAddress}
                                </span>
                                <button
                                    className='ml-5 text-main hover:underline font-medium'
                    ...
                    <div className='overflow-y-auto bg-gray-100 rounded-lg px-8 py-5 mt-5'>
                        <h1 className='text-xl font-semibold text-main mb-3'>
                            <i className='fa-solid fa-clipboard-list mr-3'></i>Order Products
                        </h1>
                        ...
                        {cart?.map((item, index, arr) => {
                            totalPrice += item.product?.variant.reduce(
                                (value, variant) =>
                                    variant._id === item.variant ? variant.price * item.quantity : value,
                    ...
                    <div className='mt-5 bg-gray-100 px-8 py-5 rounded-lg'>
                        <h1 className='text-xl font-semibold text-main'>
                            <i className='fa-solid fa-file-invoice-dollar mr-3'></i>Payment method
                        </h1>
                        <div className='my-5 flex flex-col gap-y-2'>
                            {paymentMethod.map((item) => (
                                <div key={item.id} onClick={() => setPayment(item.value)} className='flex'>
                                    <span className='text-main mr-3 cursor-pointer'>
                                        <i
                                            className={
                                                item.value === payment
                                    ...
                                    <img
                                        src={item.icon}
                                        className='w-12 h-6 object-contain bg-gray-200 mr-3 rounded'
                                        alt=''
                                    />
                                    <label className='cursor-pointer font-semibold tracking-wide' htmlFor={item.id}>
                                        {item.text}
                                    </label>
                                </div>
                            ))}
                        ...
                        <span className='italic text-red-400'>
                            {paymentMethod[paymentMethod.findIndex((item) => item.value === payment)].description}
                        </span>
                    </div>
                    <div className='mt-5 w-auto flex justify-center'>
                        {payment === 'paypal' ? (
                            ...
                            <button
                                onClick={() => {
                                    payment === 'cod' ? handleCreateOrder() : handleVnpay()
                                }}
                                className='px-4 py-3 bg-main text-white rounded-md text-lg'
                            >
                                Order Now
                            </button>
                        )}
                    ...
                    {modalAddress && (
                        <AddressList
                            ...
                            setModalUpdateAddress={setModalUpdateAddress}
                        />
                    )}
                    {modalAddAddress && (
                        <AddAddress
                    ...
                    {modelUpdateAddress && (
                        <AddAddress
                            ...
                            current={currentEdit}
                            setModalAddress={setModalAddress}
                            setModalAddAddress={setModalUpdateAddress}
                        />
                    )}
                </>
            )}
